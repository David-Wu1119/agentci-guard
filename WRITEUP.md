...
Read this honestly: the medium findings — unpinned actions, a provider key present in the job — are near-universal hygiene items, not alarms. The signal worth acting on is the small **critical** set: an AI agent with repo-write scope, on an untrusted trigger, with untrusted event content reaching it.
...
After those three fixes: **59 → 13 criticals.** I am leading with this because it is the point. A security scanner that has never embarrassed its author has not been tested against reality. The 46 that disappeared were false alarms, and a tool you can only trust after it has been forced to retract its own over-counting is more trustworthy than one that never has.
